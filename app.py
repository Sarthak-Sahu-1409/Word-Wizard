...
class WordWizard(ctk.CTk):
    ...
    def _move_item(self, direction: int):
        ...
        if 0 <= new_idx < len(self.appendix_list):
            ...
            self.appendix_listbox.see(new_idx)
    ...
    def _rename_appendix(self, event=None):
        ...
        dialog = ctk.CTkInputDialog(title="Rename Appendix", text="Enter the new appendix title:")
    ...
    def _configure_pages(self):
        ...
        dialog = ctk.CTkInputDialog(title="Configure Page Range", text=f'Enter page range (e.g., "1-5, 8, 10-12").\nTotal pages: {page_count}. Leave blank for all pages.')
        if current_range:
